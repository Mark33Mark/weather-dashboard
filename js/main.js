...
function renderCities() {
  ...
  for (let i = 0; i < cities.length; i++) {
    ...
    storedCity.classList =
      "btn citySearched large dark-grey hover-light-grey col s10 margin-bottom-sml";
...
function getTodaysWeather(chosenCity) {
  ...
  if (userPref[1] === undefined || userPref[1] === null) {
    userPref[1] = "metric";
  }
  ...
  fetch(apiURL).then(function (response) {
    if (response.ok) {
      ...
      alert(
        "I can't find that city, check your spelling or possibly I just don't have data for that location."
      );
    }
  });
}
...
function renderWeather(searchResults, place, placeCode) {
  ...
  selected city timezone offset: ${
    searchResults.timezone_offset / 3600
  } hours to Universal Time Coordinated (UTC)`;
  ...
  document.getElementById(
    "today-temp"
  ).innerText = `${searchResults.current.temp.toFixed(1)} ${tempFormat}`;
  document.getElementById(
    "today-wind"
  ).innerText = `${searchResults.current.wind_speed.toFixed(1)} ${speedFormat}`;
  document.getElementById(
    "today-humidity"
  ).innerText = `${searchResults.current.humidity} %`;
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    let forecastWeatherDescription =
      searchResults.daily[i].weather[0].description;
    let forecastWeatherIcon =
      document.getElementById(elementID).children[1].children[0];
    let weatherForecastDescription = document.getElementById(
      `day${i}-description`
    );
    weatherForecastDescription.innerText = `${forecastWeatherMain} - ${forecastWeatherDescription}`;

    document.getElementById(elementID).children[0].innerText = moment
      .unix(searchResults.daily[i].dt)
      .format("ddd, " + dateFormat);
    forecastWeatherIcon.src = `./img/accuweather_icons/${forecastWeatherIconID}.png`;

    document.getElementById(elementID).children[2].children[0].innerText =
      searchResults.daily[i].temp.day.toFixed(1) + " " + tempFormat;
    document.getElementById(elementID).children[3].children[0].innerText =
      searchResults.daily[i].wind_speed.toFixed(1) + " " + speedFormat;
    document.getElementById(elementID).children[4].children[0].innerText =
      searchResults.daily[i].humidity + "% ";
  }
}
